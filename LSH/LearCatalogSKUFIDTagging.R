...
###################################################################################
######## STEP 3. TAG IDENTICAL MPN+DESCRIPTION RECORDS
###################################################################################

## 3.1 Create a key of consisting of valid MPN and Hash of description
df$MPNHashKey<-0
validMPNidx<-which(df$ValidMPN==1)
df$MPNHashKey[validMPNidx]<-paste0(df$Product.Mpn[validMPNidx],"|",
                                   df$Hash.Value[validMPNidx])


## 3.2 Retrieve Keys that have more than one occurence (i.e. dupes in data)
##     and tag them with a unique SKUFID

MPNHashDF<-as.data.frame(table(df$MPNHashKey[validMPNidx]),
                         stringsAsFactors=FALSE)

MPNHashDupeIdx<-which(MPNHashDF$Freq>1)
numDupes<-length(MPNHashDupeIdx)
df$SKUFID<-0

for(ctr in 1: numDupes){
        
        dupeKeyIdx<-MPNHashDupeIdx[ctr]
        dupeKey<-MPNHashDF$Var1[dupeKeyIdx]
        dfIdx<-which(df$MPNHashKey==dupeKey)
        df$SKUFID[dfIdx]<-SKUFID_CTR
...
###################################################################################
######## STEP 4. GROUP TAGGED & EQUAL HASH RECORDS
###################################################################################

## 4.1 Create table of unique description Hash values that repeat in dataset
descHashTable<-as.data.frame(table(df$Hash.Value),
                         stringsAsFactors=FALSE)

idx<-which(descHashTable$Freq>1)
descHashTable<-descHashTable[idx,]
numDupeDesc<-nrow(descHashTable)


descHashTable$Tagged<-0


## 4.2 Get indices of df that have SKUFID tags
tagIdx<-which(df$SKUFID>0)


## 4.3 Get indices of df that have Hashes from descHashTable that repeat
dupeHashVector<-descHashTable$Var1
dupeHashIdx<- apply(dfp[,16,drop=FALSE],1,
                    function(x){any(x%in%dupeHashVector)})
        
## 4.4 Get the indices of df that have repeating hashes + assigned SKUFIDs

idxOfInterest<-intersect(dupeHashIdx,tagIdx)

for (ctr in 1:numDupeDesc){

        hash<-descHashTable$Var1[ctr]
        descHashTable$Tagged[ctr]<-length(which(
                                        df$Hash.Value==hash & df$SKUFID>0))
