---
title: "Evaluation of VSM as a classification algorithm using rich item  data"
...
fileLocation<-"C:\\Users\\koushikk\\repos\\XeevaIDM\\AmazonDataRunBook.csv"
...
From the table above, we observe that for 3 of the 4 classification levels, accuracy monotonically reduces with increased record count. The effect of granularity (i.e. number of groups) on accuracy seems less clear. This relationship seems to change based on record count and therefore is inconclusive. The latter is a less intutive finding. 

# Conclusion and next steps
In general, a classification accuracy of 70% of better can be considered an acceptable performance threshold for a usable classifier. By that measure, we are able to conclude that VSM performs very well when trained and tested on rich item data. Subsequent analyses will need to quantify performance when VSM is employed on  just Xeeva's existing customer data, as well as combination of rich and real-life data. An added step could  be to use Micro and Macro F1 scores as single number evaluation metric (in place of just accuracy).