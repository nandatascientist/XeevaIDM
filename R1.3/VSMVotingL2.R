...
datafileName<-"c:\\idm\\ModelB\\IU\\ModelBAggregationInputNonPOIU2.csv" # source file
outputfileName<-"c:\\idm\\ModelB\\IU\\ModelBL2Output2.csv"
...
## convert the matches into Level 2 UNSPSC Values
df$R1M1<-getL2ValueVector(df$R1M1)
df$R1M2<-getL2ValueVector(df$R1M2)
df$R1M3<-getL2ValueVector(df$R1M3)

df$R2M1<-getL2ValueVector(df$R2M1)
df$R2M2<-getL2ValueVector(df$R2M2)
df$R2M3<-getL2ValueVector(df$R2M3)

df$R3M1<-getL2ValueVector(df$R3M1)
df$R3M2<-getL2ValueVector(df$R3M2)
df$R3M3<-getL2ValueVector(df$R3M3)


## Melt dataframe by all numeric values
meltdf<-melt(df,id=c(1))

## Create a key value as combo of RecordId and L2Family as a factor
